      ...
      if (response.status === 200 && response.data.url) {
        ...
        window.addEventListener('message', async (event) => {
          ...
          if (event.data.type === `${provider}-auth-success`) {
            ...
            return navigate(`/signup?email=${event.data.user.email}&firstName=${event.data.user.firstName}&lastName=${event.data.user.lastName}&picture=${event.data.user.picture}`);
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="mb-6">
                ...
                <div className="relative">
                  ...
                  >
                    {showPassword ? (
                      <FaEyeSlash className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-400" />
                    ) : (
                      <FaEye className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-400" />
                    )}
                  </button>
                </div>
                {errors.password && (
